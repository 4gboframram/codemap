...
use codemap2::{CodeMap, File, FileData};
...
pub struct Emitter<'a, T: FileData> {
    dst: Destination<'a>,
    cm: Option<&'a CodeMap<T>>,
}

struct FileWithAnnotatedLines<T: FileData> {
    file: Arc<File<T>>,
    lines: Vec<Line>,
...
impl<'a, T: FileData> Emitter<'a, T> {
    ...
    fn preprocess_annotations(cm: Option<&'a CodeMap<T>>, spans: &[SpanLabel]) -> Vec<FileWithAnnotatedLines<T>> {
        fn add_annotation_to_file<'a, T: FileData>(file_vec: &mut Vec<FileWithAnnotatedLines<T>>,
                                  file: Arc<File<T>>,
                                  line_index: usize,
                                  ann: Annotation) {
    ...
    fn emit_message_default(&mut self,
        ...
        // get the index for
        // let's see if not binary searching and instead searching for equality breaks things
            if let Some(pos) = annotated_files.iter().position(|x| x.file.name() == primary_lo.file.name()) {
        ...
        for annotated_file in annotated_files {
            ...
            if is_primary {
                ...
                buffer.append(buffer_msg_line_offset + 1,
                              // TODO: maybe make it so it doesn't have to convert to string
